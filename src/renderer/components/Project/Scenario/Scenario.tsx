...
const Scenario: React.FC<ScenarioProps> = ({
  ...
  return (
    <div className="Scenario" key={scenario.id}>
      ...
      <div className="Scenario__section">
        <span className="Scenario__pseudo-label">Syöttötiedot</span>
        ...
        <input className="Scenario__hidden-input"
          ...
          onClick={async () => {
            const file = await openFileDialog(
              scenario.zone_data_file ? scenario.zone_data_file : projectFolder,
              [
            ...
            if (file) {
              updateScenario({ ...scenario, zone_data_file: file });
            }
          }}
        />
      ...
      <div className="Scenario__section">
        ...
        <input className="Scenario__hidden-input"
          ...
          onClick={async () => {
            const file = await openFileDialog(
              scenario.cost_data_file ? scenario.cost_data_file : projectFolder,
              [
            ...
            if (file) {
              updateScenario({ ...scenario, cost_data_file: file });
            }
          }} />
      ...
      {isFreightScenario && <div className="Scenario__section">
        ...
        <input className="Scenario__hidden-input"
          ...
          onClick={async () => {
            const file = await openFileDialog(
              scenario.trade_demand_file ? scenario.trade_demand_file : projectFolder,
              [
            ...
            if (file) {
              updateScenario({ ...scenario, trade_demand_file: file });
            }
          }}
        />
          ...
          <input id="long_dist_demand_forecast_path_input"
            ...
            onClick={async () => {
              const folder = await openFolderDialog(
                scenario.long_dist_demand_forecast_path ? scenario.long_dist_demand_forecast_path : baseDataFolder
              );
              if (folder) {
                updateScenario({ ...scenario, long_dist_demand_forecast_path: cutUnvantedCharacters(folder) });
              }
            }}
          />
        </div>
      ...
      {isPassengerTransportScenario && <div className="Scenario__section">
        ...
        <label className="Scenario__pseudo-label"
          htmlFor="iterations">Iteraatioiden enimmäismäärä;</label>
        ...
        <input id="iterations"
          ...
          onChange={(e) => {
            updateScenario({ ...scenario, iterations: parseInt(e.target.value) });
          }}
        />
        ...
        <div>
          <input type="radio" value="base" name="freight_matrix_path"
            ...
            onChange={(e) => {
              ...
              if (value === "base") {
                setGoodsTransportFreightMatrixSource(value)
          ...
          <input id="freight_matrix_path_input"
            ...
            onClick={async () => {
              const folder = await openFolderDialog(
                scenario.freight_matrix_path ? scenario.freight_matrix_path : baseDataFolder,
              );
              if (folder) {
                updateScenario({ ...scenario, freight_matrix_path: cutUnvantedCharacters(folder) });
              }
            }}
          />
        </div>
      ...
      {isPassengerTransportScenario && <div className="Scenario__section">
        <label className="Scenario__pseudo-label Scenario__pseudo-label--inline"
          ...
          />
          <span>Tallenna ajanjaksot erillisiin Emme-skenaarioihin</span>
        </label>
      </div>}
      ...
      <div className="Scenario__section">
        ...
        {isPassengerTransportScenario && <div className="Scenario__section Scenario__section--indentation">
          ...
          <input id="first-matrix-id"
            ...
            onChange={(e) => {
              updateScenario({ ...scenario, first_matrix_id: parseInt(e.target.value) });
            }}
          />
          <span style={{ color: !scenario.save_matrices_in_emme ? "#666666" : "inherit" }}
            className=" Scenario__inline">&ndash; {scenario.first_matrix_id == null ? 100 : scenario.first_matrix_id + 299}</span>
        </div>}
                ...
                <label className="Scenario__pseudo-label Scenario__pseudo-label--inline project-override-setting">
                  ...
                  <input id="override-project-folder"
                    ...
                    onClick={async () => {
                      ...
                      if (folder) {
                        updateScenario({ ...scenario, overriddenProjectSettings: { ...scenario.overriddenProjectSettings, projectFolder: cutUnvantedCharacters(folder) } });
                      }
                    }}
                  />
                </label>
                ...
                <label className="Scenario__pseudo-label Scenario__pseudo-label--inline project-override-setting">
                  ...
                  <input id="override-emme-python-path"
                    ...
                    onClick={async () => {
                      ...
                      if (file) {
                        updateScenario({ ...scenario, overriddenProjectSettings: { ...scenario.overriddenProjectSettings, emmePythonPath: cutUnvantedCharacters(file) } });
                      }
                    }}
                  />
                </label>
                  ...
                  <input id="override-valma-scripts-path"
                    ...
                    placeholder={inheritedGlobalProjectSettings.valmaScriptsPath}
                     onClick={async () => {
                      ...
                      if (folder) {
                        updateScenario({ ...scenario, overriddenProjectSettings: { ...scenario.overriddenProjectSettings, valmaScriptsPath: cutUnvantedCharacters(folder) } });
                      }
                    }}
                  />
                </label>
                    ...
                    placeholder={inheritedGlobalProjectSettings.baseDataFolder}
                     onClick={async () => {
                      ...
                      if (folder) {
                         ...
                         updateScenario({ ...scenario, overriddenProjectSettings: { ...scenario.overriddenProjectSettings, baseDataFolder: cutUnvantedCharacters(folder) } });
                      }
                    }}
                  />
                </label>
